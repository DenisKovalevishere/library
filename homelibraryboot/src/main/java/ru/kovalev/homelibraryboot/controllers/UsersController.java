...
//	@PreAuthorize("hasRole('ROLE_USER')")
	...
	public String myIndex(Model model) {
		model.addAttribute("allInfo", usersService.findMyInformation().stream().map(this::convertToInformationDTO).collect(Collectors.toList()));
...
//	@PreAuthorize("hasRole('ROLE_USER')")
	...
	public String myReadedBookInfo(Model model) {
		model.addAttribute("readedInfo", usersService.findMyInformationReadedBook().stream().map(this::convertToInformationDTO).collect(Collectors.toList()));
